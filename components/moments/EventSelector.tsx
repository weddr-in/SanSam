...
// Cinematic cover images for each event — full bleed, moody, atmospheric
...
export function MomentsEventSelector() {
  ...
  const handleSelect = (eventId: WeddingEvent) => {
    ...
    // Brief delay for the press animation to register
    setTimeout(() => {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-[#050505]">
      ...
      >
        <div>
          <p className="text-3xl text-[#d4af37]/40 tracking-wide leading-none" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontStyle: 'italic' }}>S <span className="text-[#d4af37]/20 text-xl">&</span> S</p>
        </div>
      ...
      {/* Title section */}
      <motion.div
        className="relative z-10 px-6 pt-2 pb-6 md:px-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3, duration: 0.8 }}
      >
        <h1 className="font-serif text-[42px] md:text-5xl text-[#f5f0e8] font-light leading-[1.05] tracking-tight">
          Choose<br/>your moment
        </h1>
        <div className="w-10 h-px bg-[#d4af37]/30 mt-5" />
      </motion.div>

      {/* Event cards — full-bleed cinematic posters */}
      <div className="flex-1 px-4 pb-8 md:px-6 space-y-4 md:space-y-5">
        {EVENTS.map((event, i) => (
          <motion.button
            key={event.id}
          ...
          >
            {/* Image container — 16:9 on mobile, wider on desktop */}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 md:p-7">
                <div className="flex items-end justify-between">
                  <div>
                    ...
                    <h3 className="font-serif text-3xl md:text-4xl text-[#f5f0e8] font-light leading-none
                  ...
                  {/* Enter arrow */}
                  <motion.div
                    className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/[0.1]
